...
const GeneratedVideosSection = ({
  ...
  // Handle file upload from PC - Client-side only (no server upload)
  const handleFileUpload = useCallback(
    async event => {
      ...
      try {
        ...
        // Process file completely client-side using our utility
        const processedMedia = await processMediaFile(file, 0);

        // Create uploaded image structure matching server format
        const uploadedImage = {
          _id: processedMedia._id || processedMedia.id,
          id: processedMedia._id || processedMedia.id,
          url: processedMedia.url,
          googleCloudUrl: processedMedia.googleCloudUrl,
          minUrl: processedMedia.minUrl,
          minGoogleCloudUrl: processedMedia.minGoogleCloudUrl,
          name: processedMedia.name,
          prompt: processedMedia.prompt,
          imageWidth: processedMedia.imageWidth,
          imageHeight: processedMedia.imageHeight,
          status: processedMedia.status,
        };

        console.log('✅ Processed image client-side:', uploadedImage);
